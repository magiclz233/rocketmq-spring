...
//
        //使用hashKey参数以同步方式发送请求，并接收用户类型的回复。
...
//        // 在同步模式下使用timeout和delayLevel参数参数发送请求，并接收通用类型的回复.
